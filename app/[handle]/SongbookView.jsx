...
export default function SongbookView({ songbook, channel, songs }) {
  ...
  return (
    <div className="shell">
      ...
      <section className="channel">
        ...
        {songbook.intro && <p className="intro">{songbook.intro}</p>}
        <div className="stats">
          <span>@{songbook.slug}</span>
      ...
      {visible.length === 0 ? (
        ...
        <ul className="song-grid" style={{ listStyle: "none" }}>
          {visible.map((song) => (
            ...
            >
              {song.jacketUrl ? (
                <img className="jacket" src={song.jacketUrl} alt="" />
              ) : (
